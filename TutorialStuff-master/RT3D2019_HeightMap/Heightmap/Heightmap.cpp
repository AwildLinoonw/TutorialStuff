...
bool HeightMapApplication::HandleStart()
{
	...
	m_HeightMapVtxCount = (m_HeightMapWidth - 1) * m_HeightMapLength * 2 + (m_HeightMapLength * 4);
	m_pMapVtxs = new Vertex_Pos3fColour4ubNormal3f[m_HeightMapVtxCount];
	...
	for (int y = 0; y < m_HeightMapLength - 1; ++y)
	{
		...
		if (y > 0)
		{
			m_pMapVtxs[VertexCounter++] = Vertex_Pos3fColour4ubNormal3f(PrevVerts[0], MAP_COLOUR, m_pMapVtxs[VertexCounter - ((m_HeightMapWidth * 2) + 3)].normal);
		}
		else
		{
		...
		for (int x = 0; x < (m_HeightMapWidth - 1); ++x)
		{
			...
			if (y > 0)
			{
				m_pMapVtxs[VertexCounter++] = Vertex_Pos3fColour4ubNormal3f(Vert, MAP_COLOUR, m_pMapVtxs[VertexCounter - ((m_HeightMapWidth * 2) + 3)].normal);
			}
			else
			{
